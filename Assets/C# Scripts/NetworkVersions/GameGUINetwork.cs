...
public class GameGUINetwork : NetworkBehaviour {
	...
	void Update () {
		...
		if(Input.GetKeyUp(KeyCode.Mouse0)){
			...
			if(newBox!=null){
					...
					newBox.GetComponent<SelectionBoxNetwork>().enabledBox = true;
				}
		}
		...
		if(moving==true){
			if(selectedObjects.Count>0){
				foreach(GameObject selected in selectedObjects){
					if(selected.GetComponent<StatsNetwork>().structure==false){
						selected.GetComponent<StatsNetwork>().startMove(movementWaypoint);
					}
				}
			}
			moving = false;
	...
	void selectionMenuFunction(int id){
		...
		if(showUnitButton==true){
			if(GUILayout.Button(selectedObjects[0].GetComponent<StatsNetwork>().createdObjects[0].GetComponent<SpriteRenderer>().sprite.texture)){
				GameObject.Instantiate(selectedObjects[0].GetComponent<StatsNetwork>().createdObjects[0], (selectedObjects[0].transform.position+ Vector3.left),Quaternion.identity);
				gold-=selectedObjects[0].GetComponent<StatsNetwork>().createdObjects[0].GetComponent<StatsNetwork>().cost;
			}
		}
	...
	public void newSelectionFunction(){
		if(selectedObjects.Count==1){
			...
			if(selectedObjects[0].GetComponent<StatsNetwork>().createdObjects.Count>0){
				showUnitButton = true;
			} else {
				showUnitButton = false;
			}
		...
		if(selectedObjects.Count>1){
			...
			foreach(GameObject selectedObject in selectedObjects){
				if(selectedObject.GetComponent<StatsNetwork>().structure == true){
					structureCount++;
				} else if (selectedObject.GetComponent<StatsNetwork>().structure == false){
					unitCount++;
				}
